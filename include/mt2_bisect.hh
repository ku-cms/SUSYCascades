...
inline void mt2::set_momenta(double* pa0, double* pb0, double* pmiss0)
{
...
inline void mt2::set_mn(double mn0)
{
...
inline int mt2::nsols_massless(double Dsq)
{
  double delta;
  delta = Dsq/(2*Easq);
  d1    = d11*delta;
...
inline int mt2::find_high(double & Deltasq_high)
{
   double x0,y0;
   x0 = (c1*d1-b1*e1)/(b1*b1-a1*c1);
   y0 = (a1*e1-b1*d1)/(b1*b1-a1*c1);
...
inline int mt2::scan_high(double & Deltasq_high)
{
   int foundhigh = 0 ;
   int nsols_high;
...
inline int mt2::nsols(  double Dsq)
{
