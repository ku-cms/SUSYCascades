...
void ReducedNtuple<Base>::FillOutputTree(TTree* tree, const Systematic& sys, bool do_slim){
  ...
  if(!AnalysisBase<Base>::IsData()){
    ...
    // new LepSF here
    m_elBLPoverCOLSFweight = AnalysisBase<Base>::Get_El_BLP_over_COL(Electrons+LowPtElectrons, 0);
    m_elBLPoverCOLSFweight_up = AnalysisBase<Base>::Get_El_BLP_over_COL(Electrons+LowPtElectrons, 1);
    m_elBLPoverCOLSFweight_down = AnalysisBase<Base>::Get_El_BLP_over_COL(Electrons+LowPtElectrons, -1);
